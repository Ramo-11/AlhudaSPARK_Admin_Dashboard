...
exports.create = async (req, res) => {
  try {
    ...
    const teamFolderName = `${req.body.teamName.replace(/[^a-zA-Z0-9]/g, '_')}_${Team.generateTeamId()}`;

    logger.info(`photoFile for player ${i}:`, photoFile);
    for (let i = 0; i < players.length; i++) {
      const photoFile = req.files?.find(file => file.fieldname === `players[${i}][idPhoto]`);
      ...
      if (photoFile) {
        ...
        const uploadResult = await uploadImageToCloudinary(photoFile.buffer, {
          folder: `alhuda_spark/${teamFolderName}/${playerName.replace(/[^a-zA-Z0-9]/g, '_')}_${i + 1}`,
          transformation: [
  ...
  try {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      if (photoFile) {
        ...
        // Upload new photo
        const teamFolderName = `${req.body.teamName.replace(/[^a-zA-Z0-9]/g, '_')}_${req.params.id}`;
        const playerName = players[i].playerName || `player_${i + 1}`;
        const uploadResult = await uploadImageToCloudinary(photoFile.buffer, {
          folder: `alhuda_spark/${teamFolderName}/${playerName.replace(/[^a-zA-Z0-9]/g, '_')}_${i + 1}`,
          transformation: [
